...
import { FileUpload } from '@/components/admin/Field';
...
export default function PanelSettingsPage() {
  ...
  const set = (patch: Partial<PanelSettings>) => setS((cur) => ({ ...cur, ...patch }));
  ...
  const selectTpl = (id: string) => {
    ...
    setTplId(id); setSubject(t.subject); setBody(t.body);
  };
  ...
  return (
    <div className="space-y-5">
      ...
      {tab === 'settings' && (
        <div className="max-w-3xl space-y-6 rounded-2xl border border-bd bg-white p-6 shadow-soft">
          ...
          <div className="grid gap-4 sm:grid-cols-3 text-sm">
            <div>
              <span className="mb-1.5 block font-semibold text-dteal">Panel Languages</span>
              <ul className="list-disc pl-5 text-ink">{s.languages.map((l) => <li key={l}>{l}</li>)}</ul>
            </div>
            <div>
              <span className="mb-1.5 block font-semibold text-dteal">Cint Panel Currency</span>
              <p className="text-ink">{s.cintCurrency}</p>
            </div>
            <div>
              <span className="mb-1.5 block font-semibold text-dteal">MyVoice Panel Currency</span>
              <p className="text-ink">{s.myvoiceCurrency}</p>
            </div>
          </div>

          <FileUpload label="Logo" currentSrc={asset('/assets/logo.webp')} />

          {/* Merged Rewards Info group */}
          <div className="space-y-4 border-t border-bd pt-6">
            ...
            <div>
              <span className="mb-1.5 block text-sm font-semibold text-dteal">Rewards Info</span>
              <RichTextEditor value={s.rewardsInfoHtml} onChange={(html) => set({ rewardsInfoHtml: html })} />
            </div>
          </div>
      ...
      {tab === 'templates' && (
        <div className="grid gap-4 rounded-2xl border border-bd bg-white p-4 shadow-soft lg:grid-cols-[280px_1fr]">
          <ul className="max-h-[560px] overflow-y-auto rounded-xl border border-bd">
            {data.templates.map((t) => (
              <li key={t.id}>
                <button type="button" onClick={() => selectTpl(t.id)} className={`block w-full border-b border-bd px-3 py-2.5 text-left text-sm last:border-0 ${t.id === tplId ? 'bg-lteal font-semibold text-dteal' : 'text-ink hover:bg-cream'}`}>
                  {t.name}
                </button>
              </li>
            ))}
